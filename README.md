...
- **워크스페이스** — 루트 아래 git 저장소를 모아 보고, main 과 worktree 를 골라 파일을 연다. 브랜치 줄 아래 소스 컨트롤에서 Pull · Commit · Push 를 누른다. 커밋 메시지는 그 시각(KST, `YYYY-MM-DD HH:MM:SS`)이다.
- **마크다운 편집** — `.md` 를 미리보기·원문·편집 3탭으로 다루고, 충돌을 감지해 저장한다. ` ```pug-frame ` 블록은 [pug-frame](https://github.com/dorage/pug-frame) 와이어프레임으로 그린다.
- **코드 색칠** — 원문 탭과 마크다운 코드 블록에 문법 색을 입힌다. 테마는 `/settings` 에서 고른다.
- **HTML 미리보기** — `.html` 은 격리된 iframe 에서 실제 페이지로 그린다. 옆에 둔 CSS·이미지도 함께 뜬다. 문서의 스크립트는 이 앱의 권한을 얻지 못한다.
...
- **성능 모니터링** — 이 기기의 CPU·메모리·온도와 상위 20 프로세스를 본다. `top` 을 띄우려고 SSH 하지 않는다.
...
의존성은 React 와 pug-frame 뷰어(`@pug-frame/canvas`) 둘이다. 서버 프레임워크도, SQLite 드라이버도, 차트 라이브러리도, 문법 하이라이터도 쓰지 않는다.
pug-frame 뷰어는 메인 번들에 들어가지 않고, 격리된 iframe 이 필요할 때만 받아 간다.
성능 지표도 `ps` 나 `top` 을 부르지 않고 `/proc` 를 직접 읽는다.
...
| 경로 | 내용 |
| --- | --- |
| `/` | 대시보드 — 같은 호스트의 다른 서비스 바로가기, 통계 타일, 시스템 게이지, 최근 세션·프로젝트·자주 쓴 툴·자주 쓴 스킬·최근 프롬프트 |
| `/files` | 파일 탐색기와 마크다운 에디터 |
| `/workspace` | 워크스페이스의 git 저장소. 저장소를 고르고 main·worktree 를 드롭다운으로 바꿔 가며 파일을 읽는다 |
...
| `/system` | 이 기기의 성능. CPU·코어별 사용률·메모리·스왑·온도와 CPU/메모리 상위 20 프로세스 |
| `/settings` | 코드 색 테마 고르기. 이 브라우저에만 저장된다 |
