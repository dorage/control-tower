...
- `src/domain/*`는 타입만 export 한다. 로직을 두지 않는다. 서버와 웹이 공유하는 타입은 **반드시 여기에 둔다** — 서비스에 정의하고 웹이 `import type` 으로 끌어오면 계층이 뚫린다.
- `src/lib/*`는 도메인 지식이 없는 순수 유틸만 둔다. 유일한 예외는 `http.ts`가 요청 로깅 스위치 하나 때문에 `config.ts`를 import 하는 것이다(`config.ts`는 잎 모듈이라 순환이 없다).
- `src/web/*`는 서버 코드를 import 하지 않는다. 단, `src/domain/*`의 **타입만** `import type`으로 공유한다.
...
- **`EventSource` 는 앱 전체에서 하나다.** 모듈 스코프에 두고 구독자를 참조 계수한다. 화면마다 만들면 서버에 구독자가 쌓이고, 구독자 하나마다 파일 감시 폴러가 돌아간다.
- 끊기면 `EventSource` 가 스스로 재연결한다. **재시도 루프를 직접 만들지 않는다.**
- 이벤트 핸들러는 deps 에 넣지 않는다(ref 로 최신 값을 읽는다). 넣으면 렌더마다 재구독하며 공유 연결을 흔든다.
- 각 화면은 **자체 디바운스**를 건다(2~3초). 세션이 활발하면 이벤트가 감시 주기마다 온다.
- **갱신하지 말아야 할 것을 갱신하지 않는 것이 이 기능의 전부다.**
  - 편집 중(dirty)인 문서는 **어떤 경우에도** 다시 읽지 않는다. 배너만 띄우고 결정은 사용자에게 맡긴다.
  - 목록은 첫 페이지를 보고 있을 때만 자동 갱신한다. 이어 붙인 페이지가 있으면 배너만 띄운다.
  - 타임라인은 마지막 페이지에서만 자동 갱신하고, 맨 아래에 있었을 때만 스크롤을 따라 내린다.
  - 리스트 key 는 배열 인덱스가 아니라 안정적인 식별자(`uuid ?? index`)로 준다. 자동 갱신 때 펼침 상태가 풀리지 않게.
