...
웹으로 보는 로컬 관제탑(control tower). 네 가지를 한 화면에서 제공한다.

1. **파일 탐색기** — 설정된 워크스페이스 루트의 디렉터리 트리를 본다.
2. **워크스페이스** — 루트 아래 git 저장소를 모아 보고, main 작업 트리와 worktree 를 골라 그 안의 파일을 연다. 소스 컨트롤 섹션에서 pull · commit(KST 시각 메시지) · push 를 누른다.
3. **마크다운 에디터** — `.md` 파일을 열어 편집·저장한다.
4. **세션 뷰** — `~/.claude` 아래의 Claude Code 세션을 목록/타임라인으로 본다.
...
`/api/workspace/repos`). pull 을 손으로 했으면 `bun run restart` 를 잊지 않는다. 워크스페이스 화면의
Pull 버튼도 같다 — 그래서 서버 자신의 저장소를 고르면 섹션이 그 사실을 알려 준다(T-032).
