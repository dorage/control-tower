...
2026-09-26T04:00:00Z NOTE T-032 "이슈 #28 - pull 뒤 열려 있는 파일 본문이 옛 내용으로 남던 한계를 없앤다. FileView 에 reloadSignal prop 을 두고 워크스페이스 페이지의 refreshToken 을 넘긴다. 실시간 갱신과 같은 절대 규칙 - 편집 중이면 읽지 않고 다시 읽기 배너만. 첫 렌더 값은 신호가 아니라 파일을 막 열었을 때 두 번 읽지 않는다. 새로고침 버튼도 같은 토큰이라 파일까지 다시 읽는다. 실측(헤드리스 Chromium, 4342, 클론 둘) - 깨끗할 때 pull → 본문 # before → # after-1, 배너 없음. 편집 중 pull → textarea 그대로, 배너 뜸, 다시 읽기 → # after-2. 새로고침 → # after-3. 기존 작업 개선이라 새 ID 없이 NOTE"
2026-09-26T04:02:14Z NOTE T-027 "이슈 #31 - PR #8 머지 때 T-026 의 09-03 10:36 로그 세 줄이 T-027 12:20 뒤에 놓여 check-docs 의 타임스탬프 역행 검사에 걸렸다. 9월 21일 21:15 이후 main 자동 배포가 세 번(9/24 두 번, 9/26 한 번) check 실패로 되돌려졌고 사람이 pull·restart 로 올렸다. 세 줄을 시간순 자리(T-027 앞)로 옮겼다. 머지 전 check 를 도는 CI 가 없어 이런 상태가 main 에 들어올 수 있다는 것은 남은 한계다"
