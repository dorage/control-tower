...
2026-08-31T22:20:00Z START T-016 "세션 상세를 대화 중심으로 — 사용자 요청: 툴 사용 기록은 필요 없고 유저 입력과 모델 답변이 중요하다"
2026-08-31T22:20:01Z NOTE T-016 "실측 근거: 트랜스크립트 735줄 중 사람이 쓴 프롬프트 2줄, 모델 답변 20줄. 나머지 713줄이 툴 왕복(254)·사고 과정(37)·주입된 컨텍스트(attachment 136, 그중 total_tokens_reminder 565건이 전체 프로젝트 기준 최다)·이벤트다. 기본값을 대화만으로 뒤집으니 735 → 22"
2026-08-31T22:20:02Z NOTE T-016 "블록 필터를 클라이언트에서 서버로 옮겼다. 클라이언트에서 걸러 내면 남는 블록이 없는 엔트리(툴 결과만 담긴 user 레코드)가 total 에는 세어지고 화면에는 없어서, 200개 페이지에 20개만 뜨고 페이지 경계 표시가 거짓말을 한다"
2026-08-31T22:20:03Z NOTE T-016 "system(훅 요약·턴 소요)과 attachment(리마인더·스킬 목록)를 events 쪽으로 옮겼다. 본문은 있지만 사람도 모델도 쓰지 않은 줄이다. events=1 이면 본문 그대로 되살아난다"
2026-08-31T22:20:04Z DONE T-016 "기본값 = 대화만(사고·툴·이벤트·서브에이전트 모두 꺼짐). 네 토글은 URL 에 남아 공유된다. 서버 왕복 검증: 735 → 기본 22, tools+thinking 313, 전체 735. 테스트 6종 추가"
2026-08-31T22:20:05Z NOTE T-016 "isMeta 레코드도 대화에서 뺐다. 실측 5건 전부가 인터럽트 리마인더·/context 출력·local-command caveat 였고, 사람이 쓴 프롬프트는 하나도 없었다. 요약 쪽 isUserPrompt 가 이미 같은 이유로 빼고 있던 선례를 타임라인에도 맞췄다"
