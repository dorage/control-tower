...
/** 정규식 라우터를 만들지 않는다. 경로가 일곱 개뿐이라 세그먼트 분해로 충분하다. */
function titleFor(segments: string[]): string {
  ...
  if (segments[0] === "workspace") return "워크스페이스 · control tower";
...
export function App() {
  ...
  else if (segments[0] === "workspace" && segments.length === 1) content = <WorkspacePage />;
